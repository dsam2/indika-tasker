      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 space-y-10 w-full min-w-0">
          <section className="bg-white border-4 border-black p-4 md:p-6 shadow-[8px_8px_0px_0px_rgba(0,0,0,1)]">
            <h2 className="text-xl font-black uppercase mb-4 text-black text-left">Quick Add Task</h2>
            <AddTaskForm /> 
          </section>
              ...
              {todayTasks.length > 0 ? (
                ...
                <div className="border-4 border-dashed border-black/20 p-10 flex flex-col items-center justify-center bg-white/50 space-y-4">
                  <div className="bg-yellow-300 border-4 border-black p-2 rotate-2 shadow-[4px_4px_0px_0px_rgba(0,0,0,1)]">
                    <span className="font-black uppercase text-xl italic text-black">All Sectors Clear</span>
                  </div>
                  ...
                  <div className="text-4xl animate-bounce pt-2 text-black">↑</div>
                </div>
              )}
            </div>
          </div>
          ...
          <div className="space-y-4">
            ...
            <div className="grid gap-4 w-full">
              {upcomingTasks.length > 0 ? (
                ...
                <div className="border-4 border-dashed border-black/20 p-8 flex flex-col items-center justify-center bg-white/30 text-black">
        ...
        <div className="w-full">
          <div className="bg-white border-4 border-black p-6 shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] lg:sticky lg:top-24">
            <h3 className="font-black uppercase text-sm mb-4 text-black underline decoration-pink-500 decoration-4 text-left">System Status</h3>
            <div className="space-y-6">
              ...
              <div className="pt-4 border-t-2 border-black flex flex-col gap-2">
                ...
                <p className="text-sm font-bold uppercase text-black text-left">
...
// THE HELPER FUNCTION - ESSENTIAL TO PREVENT ERRORS
function TaskCard({ task }: { task: any }) {
  const toggleWithId = toggleTask.bind(null, task.id);
  const deleteWithId = deleteTask.bind(null, task.id);

  const priorityStyles: Record<string, string> = {
    URGENT: "border-red-600 bg-red-50 shadow-red-600",
    MISSION: "border-black bg-white shadow-black",
    STASH: "border-cyan-500 bg-cyan-50 shadow-cyan-500",
  };

  const currentStyle = priorityStyles[task.priority] || priorityStyles.MISSION;
    ...
    <div className={`border-4 p-4 flex justify-between items-center transition-all duration-200
      ...
      <div className="flex items-center gap-4">
        <form action={toggleWithId}>
          <button type="submit" className={`w-8 h-8 border-4 border-black font-black ${task.completed ? "bg-green-400" : "bg-white"}`}>
            {task.completed ? "✓" : ""}
          </button>
        </form>
        <div className="flex flex-col text-left text-black">
          <div className="flex items-center gap-2">
            ...
            <span className={`font-bold uppercase tracking-tight text-lg ${task.completed ? "line-through decoration-4" : ""}`}>
              {task.title}
            </span>
          </div>
          <span className="text-[10px] font-black text-gray-400 uppercase">Due: {task.dueDate ? new Date(task.dueDate).toLocaleDateString() : "No Deadline"}</span>
        </div>
      </div>
      <form action={deleteWithId}>
        <button className="bg-red-500 text-white border-2 border-black p-2 shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] active:shadow-none">🗑️</button>
      </form>
    </div>
